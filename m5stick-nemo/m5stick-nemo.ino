...
void setup() {
  Serial.begin(9600);
  Serial2.begin(HC12_BAUD, SERIAL_8N1, HC12_SERIAL_RX, HC12_SERIAL_TX);

  if (!SD.begin(SD_CLK_PIN, SD_MISO_PIN, SD_MOSI_PIN, SD_SCK_PIN)) {
...
void loop() {
  if (Serial2.available()) {
    ...
    if (command == "Elenco") {
      listFiles();
    } else if (command.startsWith("Apri")) {
      String filename = command.substring(5);
      readFileAndTransmit(filename);
    }
  }
}

void listFiles() {
  Serial.println("Elenco dei file sulla scheda SD:");
  File root = SD.open("/");
...
void readFileAndTransmit(String filename) {
  File dataFile = SD.open(filename);
  
  if (dataFile) {
    Serial.println("Trasmissione del file " + filename + " in corso...");
